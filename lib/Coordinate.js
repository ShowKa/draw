...
class Coordinate {
    ...
    createOrigin() {
        return this.createPoint(0, 0, 0);
    }

    createLine(vector, point) {
        return new Line(vector, point);
    }

    createWall(vector, point) {
        return new Wall(vector, point);
    }

    createVectorBetween(p1, p2) {
        return new Vector(p1.x - p2.x, p1.y - p2.y, p1.z - p2.z);
    }

    createLineBetween(p1, p2) {
        let v = this.createVectorBetween(p1, p2);
    ...
    createCrossPoint(line, wall) {
        ...
        // t = - n * ( a - q ) / n * p 
        // x = a - { n * ( a - q ) / n * p } * p
        let v = p.multiply(n.innerProduct(a.differ(q)) / n.innerProduct(p));
        return this.createPoint(a.x - v.elements[0], a.y - v.elements[1], a.z - v.elements[2]);
    }
